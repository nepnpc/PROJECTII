...
// Fetch order items with both RE and LE powers
$raw_items = $pdo->query("
    ...
    if (empty($lens_ids)) {
        ...
        if ($bad) {
            ...
            foreach (array_keys($lens_ids) as $i) {
                ...
                $dt       = $dtypes[$i] ?? 'OTHER';
                $desc     = trim($descs[$i] ?? '');
                $bill_ref = $order_bill ?: null;
                $base_note = ($type_labels[$dt] ?? $dt)
                           . ($order_bill ? ' [Order: ' . $order_bill . ']' : '')
                           . ($desc ? ': ' . $desc : '');

                // Deduct inventory based on eye side
                if ($eye_side === 'RE' || $eye_side === 'BOTH') {
                    $ledger_stmt->execute([$lid, $re_sph, $re_cyl, $axis, -$qty,
                                           $base_note . ' [RE]']);
                    $ledger_id = (int)$pdo->lastInsertId();
                }
                if ($eye_side === 'LE' || $eye_side === 'BOTH') {
                    $ledger_stmt->execute([$lid, $le_sph, $le_cyl, $axis, -$qty,
                                           $base_note . ' [LE]']);
                ...
                $dmg_stmt->execute([
                    ...
                    $axis, $qty, $eye_side, $dt, $desc, $ledger_id, $bill_ref
                ]);
                $count++;
            }
...
$damages = $pdo->query("
    ...
    FROM damages d
    JOIN lenses l ON l.id = d.lens_id
...
<div class="card" style="margin-bottom:20px;">
    <div class="card-header"><h3>&#9888; Report Damage</h3></div>
    <div class="card-body">
        ...
        <div style="margin-bottom:18px;">
            ...
            <div style="display:flex;gap:8px;align-items:center;">
                ...
                <button type="button" class="btn btn-outline btn-sm"
                        onclick="loadFromOrder(document.getElementById('bill-lookup').value)">
                    Load Order
                </button>
                <button type="button" class="btn btn-sm btn-secondary" onclick="clearRows()">
                    Clear All
                </button>
            </div>
        ...
        <form method="POST" id="damage-form">
            ...
            <div style="margin-top:10px;">
                <button type="button" class="btn btn-outline btn-sm" onclick="addDamageRow(null)">
                    &#43; Add Row
                </button>
            </div>
            <div style="margin-top:16px;">
                <button type="submit" class="btn btn-danger btn-lg" onclick="return confirmSubmit()">
...
<!-- ── DAMAGE HISTORY ──────────────────────────────────────── -->
<div class="card">
    <div class="card-header"><h3>Damage History</h3></div>
    <div class="card-body" style="padding-bottom:0;">
        <div class="action-bar" style="margin-bottom:12px;">
            <input type="search" id="dmg-search"
                   placeholder="Search brand, type, order..."
...
function onEyeSide(select, id) {
    ...
    var reDisabled = (val === 'LE');
    var leDisabled = (val === 'RE');

    reSph.disabled = reDisabled; reSph.style.opacity = reDisabled ? '0.35' : '1';
    reCyl.disabled = reDisabled; reCyl.style.opacity = reDisabled ? '0.35' : '1';
    leSph.disabled = leDisabled; leSph.style.opacity = leDisabled ? '0.35' : '1';
    leCyl.disabled = leDisabled; leCyl.style.opacity = leDisabled ? '0.35' : '1';
}
...
function filterDamages(query) {
    ...
    renderTable('damage-table-body', filtered, function(d, idx) {
        ...
        var reCell = (d.eye_side === 'LE') ? '<span style="color:#ccc;">—</span>' : reInfo;
        var leCell = (d.eye_side === 'RE') ? '<span style="color:#ccc;">—</span>' : leInfo;
